...
- Pair each unit's pass with a harsh check of its own and audit every pass's artifacts yourself before trusting them. If a pass games the gate, reset and harden the contract. If the gate itself is wrong, fix the gate in its own change rather than routing around it.
