...
Keep it tight and on-topic. Read only what the in-scope threads need, then stop. The heavy reading fans out to parallel workers; the parent keeps their findings and the final brief.
...
3. Fan out across your chat history. List candidates with `pstack_transcripts`, newest modification time first, and partition the in-scope session IDs into disjoint slices. Resolve `swarm-worker` with `pstack_route` and use a native task batch, one read-only brief per slice, without isolation. Each worker searches the topic first, reads only matching regions within the tool's bounds, and returns topic, user goal, decisions, open threads, corrections, and artifacts, each citing the session ID. Skip the current session and unrelated test noise. Collect every job through `hub` before synthesis; raw transcripts stay in the workers. For one or two sessions, skip fan-out and search directly. If a read is truncated, report the omitted coverage or request a narrower source; do not claim a complete scan.
4. Sweep the shared record whenever the topic names a feature, file, subsystem, area, or bug. This is the default, not a judgment call, and "my work on X" does not exempt it. A named target carries history you never see in your own transcripts, and that history is the point of the sweep. Hand it to the **why** skill and its per-source playbooks, but steer their question from "why was this built this way" to "what's the current state, what's been tried and didn't hold, and what are users still reporting". Reuse its per-source playbooks so you don't reinvent each query vocabulary, and run its source investigators alongside the history workers. Inherit its posture: one investigator per source, null results are findings, skip an unavailable MCP and say so. Fold what comes back into the brief. Skip this step only for pure activity recall with no named target ("what did I do this week"), where your own history and live state are the entire answer.
