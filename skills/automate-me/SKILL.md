---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via the authoring-a-skill playbook + unslop, optionally pulling fresh evidence from recent project transcripts."
...
This skill orchestrates three others: an inline mining pass (see step 1), the authoring-a-skill playbook in the **poteto-mode** skill (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.omp/skills/**/*-mode/SKILL.md` and `~/.omp/agent/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.omp/skills/<handle>/`), not only at the top level. If one exists, confirm intent in prose (unless they already said "update my skill" or similar):
...
Mining misses intent that hasn't come up yet. Ask in prose, structured so the user picks options instead of typing from scratch. Lower cognitive load, higher hit rate.
...
Author the skill following the authoring-a-skill playbook in the **poteto-mode** skill. Placement:
...
Apply the **unslop** skill and the authoring playbook's prose rules to every line. Both apply to any agent-read prose, not just skills.
...
- User wants a task-specific skill (not working conventions): skill authoring alone, no mining required.
