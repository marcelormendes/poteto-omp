...
Read `references/epistemics.md` for the full confidence framework and phrasing guide. The synthesizer pass must follow it.
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the current selection or position, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.

## Step 2. Establish the Code Anchor

Before running passes, anchor the investigation in concrete code. You need:
...
Build this inline. It's cheap, and every pass needs it.
...
Pull PR bodies and discussion via `gh` for any substantive commits; if `gh` is missing, record the gap and say the source-control pass is partial, never invent PR data:

```bash
gh pr view <number> --json title,body,author,createdAt,mergedAt,labels,closingIssuesReferences,comments,reviews
```

Capture this as seed context (file paths, symbols, commits, PR numbers, linked ticket IDs). Pass it to the passes so they don't rediscover it.

## Step 3. Run Investigator Passes (default posture)

**Default to the full coverage investigation.** Each evidence category lives in a different kind of system, and you cannot tell from the question alone which one holds the answer without looking. So look across every available category, by default.

### Discovery

List the available MCP servers first: project `.omp/mcp.json`, user `~/.omp/agent/mcp.json` (global), or `/mcp` in-session for status; MCP tools appear in the session tool list as `mcp__<server>_<tool>`. If no config file exists, the available-tools list is the source of truth for what is wired up.
...
"It's pure feature code, error tracking won't have anything" is **not** sufficient, and neither is "I doubt long-form docs would have this." Run the search; let the null result speak. The cost of a pass returning empty is a few minutes of searching. The cost of missing a design doc that actually exists is a wrong answer.
...
**Sources Consulted**. One line per pass, including the ones that returned nothing. The reader should see at a glance (a) which MCPs were queried, (b) which came back empty, and (c) which were skipped and why. This coverage map lets the user judge breadth and redirect if something obvious was missed.
...
**Pass log.** One line per pass: `Pass N — <source> — <model> — PASS/ISSUES/BLOCKED`. This records the model attribution and dropout status the reader needs to judge coverage.
...
- **Skipping passes by anticipation**. Deciding up front that "long-form docs probably don't have this" or "this isn't an error tracking thing" without searching. The default-to-all-seven posture prevents this. A null result is a data point; a skipped search is a blind spot.
- **Collapsing sources into one pass**. Each MCP has its own query vocabulary, result shape, and pitfalls; pooling them dilutes specialization and makes coverage harder to reason about. Always one pass per category.

## Reference Files

- `references/epistemics.md`. Confidence tiers and phrasing guide. The synthesizer pass must follow it.
- `references/investigator-prompt.md`. Base prompt template for each investigator pass.
- `references/source-playbook.md`. Index pointing at the category playbooks below.
- `references/sources/*.md`. One self-contained example playbook per category, plus cross-cutting `incident-postmortem.md`. Give a pass the single file that matches its category and adapt it to the available MCP.
