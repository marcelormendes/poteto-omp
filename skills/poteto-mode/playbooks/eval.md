...
4. **Launch N candidate passes** as one `task` batch per the **arena** skill's Phase B, one item per seat: `task({ context: <sanitized dirs + shared background>, tasks: [ { name: "candidate-<n>", agent: "pstack-arena-runners-<n>", task: <the same organic prompt>, isolated: true } ... ] })`. The call is the barrier and results return in input order. Each item's brief names its own sanitized dir (the dirs sit at `/tmp/<slug>/candidate-<n>/`, never worktrees of one repo, because the variant differs per candidate). Same prompt to each. Role agents are bound by `setup-pstack` from the `modelRoles` entries in `~/.omp/agent/config.yml`; fall back to the seat's default, the arena-runners list order, never invent a model selector, so the panel crosses model families, and record which model each lane ran under. When the role agents are not installed, run `setup-pstack` first, then run the candidate passes sequentially in this session, one numbered pass per candidate, and record each pass's model.
5. **Run one blinded judge pass** on a different model family per the **arena** skill's Phase C as a flat-form `task` spawn: `{ agent: "pstack-arena-cross-judges-<n>", task: <the judge brief pointing at the sanitized outputs> }`. Pick the seat whose model family differs from every candidate's; role agents are bound by `setup-pstack` from the `modelRoles` entries in `~/.omp/agent/config.yml` (fall back to the seat's default) and record it. The judge sees outputs by sanitized label and the rubric, never a model name. When the role agents are not installed, run `setup-pstack` first, then run the judge pass as a numbered sequential pass in this session, recording the judge pass's model.
