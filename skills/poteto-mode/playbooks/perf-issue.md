...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Launch the implementation pass through the `task` tool: `{ context: <shared evidence>, tasks: [{ name: "implementation", agent: "pstack-perf-issue", task: <the brief>, isolated: true }] }`; its brief runs the verification command inside the worktree before reporting. Review its diff before accepting, never accept its summary on faith. The perf role agent is a writer: isolated checkout, post-fix trace captured before the report. Role agents are bound by `setup-pstack` from the `modelRoles` entries in `~/.omp/agent/config.yml`; resolve the role with `pstack_route`; never invent a model selector. Record which model ran the launch. Capture a post-fix trace.
