...
1. Capture a baseline trace via the bash-driven harness for the surface (Non-negotiables).
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Launch the implementation pass through the `task` tool: `{ agent: "pstack-perf-issue", task: <the brief>, isolated: true }`; its brief runs the verification command inside the worktree before reporting. Review its diff before accepting, never accept its summary on faith. The perf role agent is a writer: isolated checkout, post-fix trace captured before the report. Role agents are bound by `setup-pstack` from the `modelRoles` entries in `~/.omp/agent/config.yml`; fall back to the role's default, the strongest judgment model (setup default), never invent a model selector. Record which model ran the launch. Capture a post-fix trace.
...
**Inline passes (only when the role agents are not installed; run `setup-pstack` first).** Run the same briefs sequentially in this session, one numbered pass per delegated step, per Passes in the skill, and record each pass's model. Same scope, same PASS/ISSUES/BLOCKED envelope, same dropout tolerance. Isolation via bash `git worktree add` when a pass writes; never merge without explicit user confirmation.
