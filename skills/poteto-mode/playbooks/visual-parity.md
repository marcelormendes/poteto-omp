...
3. Migrate one component at a time. Each is an independent artifact, so launch parallel workers across components, each in its own native isolation workspace (the **separate-before-serializing-shared-state** principle skill). Shared primitives migrate first as a blocking phase.
4. Verify each component against its baseline via image diff on the matching surface via the bash-driven harness. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Poll per component until the diff is zero (a bash polling loop with an explicit iteration cap).
