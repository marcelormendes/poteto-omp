...
1. Locate the prior trail. A session transcript via the **pstack_transcripts** tool for this project (do not read another project's transcripts, that crosses workspace boundaries and reads private chats from unrelated projects), or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Have a native `pstack-swarm-worker` job read a long transcript with a read-only, project-scoped brief and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
