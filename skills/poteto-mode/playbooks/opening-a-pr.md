...
**Worktree.** Work from a git worktree off main. Multiple passes on the same branch each get their own worktree, or `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.
...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the bash-driven harness (Playwright script, PTY helper, or curl), the project's verify skill, or the targeted tests. State the outcome of each check, not only the command name.
...
A pass that opens a PR runs **interrogate**, `/unslop`, and `/no-comments`. It returns the URL and does not babysit.
