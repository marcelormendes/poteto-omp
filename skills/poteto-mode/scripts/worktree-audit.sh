...
# Session transcripts dir: OMP session trees below ~/.omp/agent/sessions,
# one dir per project with one .jsonl per session. Override via PSTACK_TRANSCRIPTS_DIR.
transcripts="${PSTACK_TRANSCRIPTS_DIR:-$HOME/.omp/agent/sessions}"
now=$(date +%s)

# Portable stat/date helpers: GNU first, BSD fallback.
fstat_files() { stat -c '%Y %n' "$@" 2>/dev/null || stat -f '%m %N' "$@" 2>/dev/null; }
...
git worktree list --porcelain | awk '/^worktree /{print $2}' | while read -r wt; do
	...
	if [ -d "$transcripts" ]; then
		f=$(grep -rl -e "${wt}/" -e "${wt}\"" "$transcripts" 2>/dev/null \
			| xargs fstat_files 2>/dev/null | sort -rn | head -1)
		if [ -n "$f" ]; then last_ts=$(echo "$f" | awk '{print $1}')
			last=$(fmt_date "$last_ts"); fi
	fi
