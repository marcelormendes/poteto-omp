...
export interface RpcDriverOptions {
  ...
  onUIRequest?: (frame: Record<string, unknown>) => Record<string, unknown>;
}
...
export class RpcDriver {
  ...
  private stderrTail = "";
  private readonly chunked = new Map<
    string,
    Array<{ index: number; count: number; data: string } | null>
  >();
  ...
  async #spawn(): Promise<void> {
    ...
    const proc = spawn({
      cmd: [this.options.omp ?? process.env.PSTACK_OMP_BIN ?? "omp", ...args],
    ...
    (proc.stderr as ReadableStream<Uint8Array>)
      .pipeTo(
        new WritableStream<Uint8Array>({
          write: (chunk) => {
            this.stderrTail = (
              this.stderrTail + stderrDecoder.decode(chunk, { stream: true })
            ).slice(-4096);
          },
        }),
      )
    ...
    proc.exited.then((code) => {
      ...
      for (const [id, request] of this.pending) {
        ...
        request.reject(
          new RpcDriverError(
            `OMP exited (code ${code}) while awaiting ${request.command}`,
            "transport",
            tail,
          ),
        );
  ...
  #dispatch(frame: Record<string, unknown>): void {
    ...
    if (
      !["message_update", "tool_execution_update"].includes(String(frame.type))
    )
    ...
    if (frame.type === "extension_ui_request" && typeof frame.id === "string") {
      this.#writeFrame({
        ...
        id: frame.id,
        ...(this.options.onUIRequest?.(frame) ?? { cancelled: true }),
      });
      return;
    }
  ...
  #reassembleChunk(
    ...
    const pieces =
      this.chunked.get(chunkId) ??
      new Array<{ index: number; count: number; data: string } | null>(
        count,
      ).fill(null);
    ...
    const json = Buffer.concat(
      pieces
        .filter(
          (piece): piece is { index: number; count: number; data: string } =>
            piece !== null,
        )
        .sort((a, b) => a.index - b.index)
        .map((piece) => Buffer.from(piece.data, "base64")),
    ).toString("utf8");
    try {
  ...
  #writeFrame(frame: Record<string, unknown>): void {
    ...
    const stdin = this.process_?.stdin as unknown as
  ...
  async #negotiate(): Promise<void> {
    ...
    if (
      !response.success ||
      (response.data as { protocolVersion?: number } | undefined)
        ?.protocolVersion !== 2
    ) {
      throw new RpcDriverError(
        `RPC negotiation failed: ${response.error ?? "unknown"}`,
        "negotiation",
        this.stderrTail,
      );
    }
  ...
  async send(
    ...
    if (this.closed)
      throw new RpcDriverError(
        "OMP transport is closed",
        "transport",
        this.stderrTail,
      );
    const id =
      typeof command.id === "string" ? command.id : `rpc-${++this.nextId}`;
    ...
    const promise = new Promise<RpcResponse>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new RpcDriverError(
            `Timed out waiting for response to ${String(command.type)}`,
            "command",
            this.stderrTail,
          ),
        );
  ...
  async promptAndWait(
    message: string,
    timeoutMs = 300_000,
  ): Promise<{ agentInvoked: boolean; frames: Record<string, unknown>[] }> {
    ...
    const { promise: result, resolve: resolvePromise } = Promise.withResolvers<{
    ...
    try {
      ...
      this.#requireSuccess(response, "prompt");
  ...
  async getAvailableCommands(): Promise<RpcAvailableCommand[]> {
    ...
    const commands =
      (response.data as { commands?: RpcAvailableCommand[] } | undefined)
        ?.commands ?? [];
    return commands;
  }
  ...
  async getSubagents(): Promise<RpcSubagentSnapshot[]> {
    ...
    const subagents =
      (response.data as { subagents?: RpcSubagentSnapshot[] } | undefined)
        ?.subagents ?? [];
    return subagents;
  }
  ...
  async currentSessionId(): Promise<string> {
    ...
    if (typeof sessionId !== "string" || sessionId.length === 0) {
      throw new RpcDriverError(
        "get_state returned no sessionId",
        "state",
        this.stderrTail,
      );
    }
  ...
  #requireSuccess(response: RpcResponse, command: string): void {
    if (!response.success) {
      throw new RpcDriverError(
        `${command} failed: ${response.error ?? "unknown"}`,
        command,
        this.stderrTail,
      );
    }
